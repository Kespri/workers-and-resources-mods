...
|---|---|---|---|
| `plugins/deposits_plus` | 1.8.0-beta | deposit types, natural generation, sandy meadow surface, working vehicles | fork of the `deposits` plugin from the TesmioLoader by MaxLegend (Tesmio), GPL v3 |
...
| `plugins/localization` | 1.2 | translation service: text packs in INI files become extended language files in the VFS | own work; ships the text packs `research_expansion`, `technical_service_storage`, `tesmio_lang` |
| `plugins/rail_physics_fix` | 1.3.4-beta | physical train dynamics: traction, braking, grade, curves, stations, customs, consumption | fork of [RailPhysics 1.3.0](https://github.com/TheRealMeowMeow00/WRSR_RailPhysics) by Meow Meow (TheRealMeowMeow00), GPL v3; [original on the Workshop](https://steamcommunity.com/sharedfiles/filedetails/?id=3776784867) |
| `plugins/research_expansion` | 1.5 | new research and changes to the research tree through the VFS | own work |
| `plugins/resources_button_fix` | 1.1 | compact button grids in the terrain editor | own work |
| `plugins/technical_service_storage` | 0.3.1-beta | grit storages, material priorities and snowplow tanks in the Technical Services | own work |
| `plugins/ui_layout_fixes` | 1.1 | layout corrections in game windows | own work |
| `plugins/vanilla_buildings` | 1.3 | patches for Vanilla, DLC and Workshop buildings without touching the original files | own work |
| `plugins/vehicle_materials` | 1.2.0-beta | material demand of vehicles | own work |
| `plugins/weather_roads` | 0.2.11-beta | road snow, melting and protection after plowing | own work |
| `plugins/workshop_bridge` | 0.1.0-beta | loads Workshop hook DLLs without Soviet Mod Loader | own work |
