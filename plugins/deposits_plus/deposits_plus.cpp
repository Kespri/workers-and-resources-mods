// deposits_plus - deposit types the base game does not have, as a tesmioloader
// plugin. One declaration also opts a custom mine into native working vehicles.
//
...
// deposits_plus.ini declares them and this one table feeds all three:
//
...
// deposits_plus.ini declared - and carry its own per-deposit keys in the same file.
// `depletion` does both.
//
...
namespace DG = deposit_generation;
// The DLL's own folder (set in DllMain); empty against a host that loaded us oddly.
static char g_selfDir[MAX_PATH];
static bool g_generationEnabled = true;
...
// ---------------------------------------------------------------- deposit registry
//
// Nothing about any individual deposit is compiled in. deposits_plus.ini declares
...
// deposits_plus.ini for which are actually free.
...
struct DepositDef
{
    ...
    int   workingVehicleSkill; // 0 = unchanged, 3 = native GRAVELMINING
...
static bool GenerationNumber(const char* value, double low, double high, double* out)
{
    errno=0; char* end=NULL; double n=strtod(value,&end);
    if(errno || end==value || *end || !std::isfinite(n) || n<low || n>high) return false;
    *out=n; return true;
}
static bool GenerationSetting(DepositDef* d,const char* key,const char* value)
{
    double n=0; bool valid=true,known=true;
    if(!d) {
        if(KeyIs(key,"generation")) { valid=GenerationNumber(value,0,1,&n) && n==floor(n); if(valid) g_generationEnabled=n!=0; }
        else if(KeyIs(key,"generate_existing_empty")) { valid=GenerationNumber(value,0,1,&n) && n==floor(n); if(valid) g_generateLegacyEmpty=n!=0; }
    ...
    if(known && !valid) {
        Logf("generation WARN [%s] invalid %s=\"%s\"; generation disabled for safety",
             d?d->name:"deposits_plus",key,value);
        if(d) { d->generation.enabled=false; d->generationInvalid=true; }
        else { g_generationEnabled=false; g_generationConfigInvalid=true; }
    }
    return known;
}

// This release deliberately supports only the excavator/bulldozer capability
// whose acceptance, work and arrival paths have all been verified. A misspelled
// or unsupported value must not turn a different machine into an excavator.
static void SetWorkingVehicleSkill(DepositDef* d, const char* value)
{
    d->workingVehicleSkillSet = true;
    d->workingVehicleSkill = 0;
    if (KeyIs(value, "gravelmining")) d->workingVehicleSkill = 3;
    else if (!KeyIs(value, "none"))
        Logf("vehicles WARN [%s] working_vehicle_skill=\"%s\" is unsupported; "
             "use none or gravelmining. Vehicle extension disabled for this deposit; "
             "its map and mining definition are kept", d->name, value);
}

static bool UsesWorkingVehicles(const DepositDef& d)
{
    return d.workingVehicleSkill == 3 && d.buildingType == 7 &&
           d.type >= 10 && d.type <= 127;
}
...
{
    char path[MAX_PATH];
    _snprintf_s(path, sizeof(path), _TRUNCATE, "%s\\plugins\\deposits_plus.ini", g_baseDir);

    HANDLE h = CreateFileA(path, GENERIC_READ, FILE_SHARE_READ, NULL,
                           OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (h == INVALID_HANDLE_VALUE)
    {
        Logf("deposits_plus  no plugins\\deposits_plus.ini - no mod deposit types");
        return;
    }

    LARGE_INTEGER length={};
    if(!GetFileSizeEx(h,&length) || length.QuadPart<0 || length.QuadPart>1024*1024) {
        CloseHandle(h); Logf("deposits_plus WARN INI unreadable or over 1 MiB; no partial registry accepted"); return;
    }
    std::vector<char> input((size_t)length.QuadPart+1,0);
    char* buf=input.data();
    DWORD got = 0;
    bool readOk=ReadFile(h,buf,(DWORD)length.QuadPart,&got,NULL)!=0 && got==length.QuadPart;
    CloseHandle(h);
    if(!readOk) { Logf("deposits_plus WARN INI read incomplete; no partial registry accepted"); return; }
    buf[got] = 0;
    ...
    for (char* line = strtok_s(buf, "\n", &ctx); line; line = strtok_s(NULL, "\n", &ctx))
    {
        ...
        if (line[0] == '[')
        {
            ...
            *end = 0;
            ...
            // [deposits_plus] is the plugin's own settings; every other section is a
            ...
            {
                Logf("deposits_plus  \"%s\" ignored - only %d sections fit", line + 1, MAX_DEPOSITS);
                continue;
            }

            d = &g_dep[g_depCount++];
            memset(d, 0, sizeof(*d));
            d->generation=DG::Settings();
            d->legacyTerrainComponent=-1;
        ...
        *eq = 0;
        ...
        Trim(line);
        Trim(val);
        if(GenerationSetting(d,line,val)) continue;
        if(!d) continue;
        ...
        else if (KeyIs(line, "working_vehicle_skill")) SetWorkingVehicleSkill(d, val);
        else if (KeyIs(line, "map"))
        {
            ...
            // every existing deposits_plus.ini already uses.
            ...
            else Logf("deposits_plus  \"%s\": unknown map \"%s\"", d->name, val);
        }
        else if (KeyIs(line, "radius"))
        {
            ...
            if (!d->radiusRva)
            {
                ...
                if (d->radiusValue <= 0.0f)
                    Logf("deposits_plus  \"%s\": unknown radius \"%s\"", d->name, val);
            }
        }
        ...
        else Logf("deposits_plus  \"%s\": no room for \"%s\" - %d plugin keys per section",
...
static void ValidateDeposits()
{
    ...
    for (int i = 0; i < g_depCount; i++)
    {
        ...
        if (bad)
        {
            Logf("deposits_plus  \"%s\" rejected: %s", d->name, bad);
            continue;
        }
    ...
    {
        DepositDef* d = &g_dep[i];
        if (d->workingVehicleSkill && d->buildingType != 7)
        {
            Logf("vehicles WARN [%s] working_vehicle_skill requires building_type=7 "
                 "(mine), got %d. Vehicle extension disabled; deposit kept",
                 d->name, d->buildingType);
            d->workingVehicleSkill = 0;
        }
    }
    ...
    for (int i = 0; i < g_depCount; i++)
    {
        ...
        if (d->map < 0)
        {
            Logf("deposits_plus  \"%s\" rejected: no free channel left in %d maps", d->name, MAX_MAPS);
    ...
    for (int i = 0; i < g_depCount; i++)
    {
        ...
        // Allocate opted-in terrain conversions AFTER legacy auto channels.
        // First adoption therefore does not shift existing clay/gas/etc slots.
        if(d->independentMap && d->map==DEP_MAP_TERRAIN) {
            int chosenMap=-1,chosenComponent=-1;
            for(int m=DEP_MAP_EXTRA;m<MAX_MAPS && chosenMap<0;++m)
                for(int c=0;c<4;++c) if(!ChannelTaken(g_depCount,m,c)) { chosenMap=m; chosenComponent=c; break; }
            if(chosenMap>=0) {
                d->legacyTerrainComponent=d->component;
                d->map=chosenMap; d->component=chosenComponent;
                Logf("generation [%s] independent map enabled; existing save terrain channel %d will be adopted without painting terrain",d->name,d->legacyTerrainComponent);
            } else {
        ...
        for (size_t k = 0; k < sizeof(kTaken) / sizeof(kTaken[0]); k++)
            if (kTaken[k].map == d->map && kTaken[k].comp == d->component)
                Logf("deposits_plus  \"%s\" WARN  shares a channel with %s - both read the same bytes",
                     d->name, kTaken[k].who);
    ...
    for (int i = 0; i < g_depCount; i++)
    {
        ...
        Logf("deposits_plus  \"%s\" type %d \"%s\" -> %s component %d, radius %s, "
    ...
    if (g_extraCount)
        Logf("deposits_plus  %d map(s) past the engine's two: resourcemap3..resourcemap%d",
...
{
    if(g_extraCount==0 && g_depCount>0 && g_generationEnabled) g_extraCount=1;
    ...
    g_generationSaveReady=PatchWorldSaveCall((void*)h_WorldSaveMaps);
    if (!g_generationSaveReady)
...
#include "deposit_generation_runtime.inl"
...
// ---------------------------------------------------------------- working vehicles
//
// WRSR 1.1.1.9 has four separate (depositType == 3 || depositType == 7)
// gates for excavators. Extend the gates, not the building's identity: changing
// +0x368 to gravel even temporarily would also change deposit sampling.
//
// 176DF8: working-vehicle contribution called from the type-7 mine tick.
// 1B169A: the corresponding inlined contribution in the alternate mine tick.
// 3E3AF9: shared vehicle eligibility (3E2900), used by purchase and assignment.
// 66E628: quarry vehicle arrival/working-state transition.
//
// Only the type predicates are replaced. Skill 3, fuel/condition checks,
// capacity, road/access rules, production formula and native work routines stay
// in the engine. No per-tick polling, new vehicle data or save format is needed.
// All four gates are preflighted and made writable BEFORE publishing any jump.

struct WorkingVehicleSite
{
    DWORD rva;
    const BYTE* original;
    size_t size;
    size_t prepareSize;       // SUB/LEA producing depositType - 3
    bool ecx;                // the predicate uses ECX here, EAX at the others
    BYTE typeDescModRm;       // CMP dword ptr [typedesc+0x360], 7
    DWORD acceptRva;
    DWORD rejectRva;
    DWORD guardRva;
    const BYTE* guard;
...
static const BYTE kVehiclePower[] = {
    0x83,0xE9,0x03, 0xF7,0xC1,0xFB,0xFF,0xFF,0xFF, 0x74,0x1B
};
static const BYTE kVehicleTick[] = {
    0x41,0x8D,0x43,0xFD, 0xA9,0xFB,0xFF,0xFF,0xFF, 0x74,0x1B
};
static const BYTE kVehicleEligibility[] = {
    0x41,0x8D,0x41,0xFD, 0xA9,0xFB,0xFF,0xFF,0xFF, 0x75,0x56
};
static const BYTE kVehicleArrival[] = {
    0x83,0xE8,0x03, 0xA9,0xFB,0xFF,0xFF,0xFF, 0x0F,0x85,0x90,0,0,0
};
static const BYTE kVehiclePowerGuard[] = {
    0x41,0x83,0x3C,0xC9,0x03, 0x75,0x18,
    0x48,0x8B,0x87,0x18,0x03,0,0, 0x8B,0x88,0x68,0x03,0,0
};
static const BYTE kVehicleTickGuard[] = { 0x41,0x83,0x3C,0xCA,0x03, 0x75,0x0B };
static const BYTE kVehicleEligibilityGuard[] = {
    0x83,0xFF,0x07, 0x0F,0x85,0xBD,0,0,0, 0x44,0x8B,0x8D,0x68,0x03,0,0
};
static const BYTE kVehicleArrivalGuard[] = {
    0x41,0x83,0xFE,0x07, 0x0F,0x85,0xA4,0,0,0, 0x8B,0x86,0x68,0x03,0,0
};
static const WorkingVehicleSite kWorkingVehicleSites[] = {
    { 0x176DF8, kVehiclePower, sizeof(kVehiclePower), 3, true, 0xB8,
      0x176E1E, 0x176E03, 0x176DE4, kVehiclePowerGuard, sizeof(kVehiclePowerGuard), "mine work sum" },
    { 0x1B169A, kVehicleTick, sizeof(kVehicleTick), 4, false, 0xBF,
      0x1B16C0, 0x1B16A5, 0x1B1693, kVehicleTickGuard, sizeof(kVehicleTickGuard), "alternate mine work sum" },
    { 0x3E3AF9, kVehicleEligibility, sizeof(kVehicleEligibility), 4, false, 0xBD,
      0x3E3B04, 0x3E3B5A, 0x3E3A94, kVehicleEligibilityGuard, sizeof(kVehicleEligibilityGuard), "purchase/assignment eligibility" },
    { 0x66E628, kVehicleArrival, sizeof(kVehicleArrival), 3, false, 0xBE,
      0x66E636, 0x66E6C6, 0x66E618, kVehicleArrivalGuard, sizeof(kVehicleArrivalGuard), "quarry vehicle arrival" }
};
static const size_t kWorkingVehicleSiteCount = sizeof(kWorkingVehicleSites) / sizeof(kWorkingVehicleSites[0]);
static BYTE* g_workingVehicleCave;
static const SIZE_T kWorkingVehicleCaveSize = 4096;

static bool VehicleRel32Fits(const BYTE* next, const BYTE* target)
{
    const __int64 distance = (__int64)(uintptr_t)target - (__int64)(uintptr_t)next;
    return distance >= (-2147483647LL - 1) && distance <= 2147483647LL;
}

static void VehicleBranch(Emit& e, BYTE* target, bool conditional)
{
    if (conditional) { e.b(0x0F); e.b(0x84); } // JE
    else e.b(0xE9);
    if (!VehicleRel32Fits(e.p + 4, target)) e.overflow = true;
    e.rel32(target);
}

static void EmitWorkingVehicleGate(Emit& e, const WorkingVehicleSite& site,
                                   BYTE* accepted, BYTE* rejected)
{
    for (size_t j = 0; j < site.prepareSize; j++) e.b(site.original[j]);
    const size_t testSize = site.ecx ? 6 : 5;
    for (size_t j = 0; j < testSize; j++) e.b(site.original[site.prepareSize + j]);
    VehicleBranch(e, accepted, true); // original gravel/bauxite path first

    // Do not extend a non-mine with a coincidentally equal subtype. These
    // descriptor registers are already live at the respective native sites.
    e.b(0x83); e.b(site.typeDescModRm); e.d32(0x360); e.b(7);
    BYTE* notMine = e.jne32();
    for (int i = 0; i < g_depCount; i++)
    {
        if (!UsesWorkingVehicles(g_dep[i])) continue;
        e.b(0x83); e.b(site.ecx ? 0xF9 : 0xF8); e.b((BYTE)(g_dep[i].type - 3));
        VehicleBranch(e, accepted, true);
    }
    e.land(notMine);
    // Preserve the original rejected-path flags as well as all registers.
    // No pushes/calls, floating-point instructions or writes in these caves.
    for (size_t j = 0; j < testSize; j++) e.b(site.original[site.prepareSize + j]);
    VehicleBranch(e, rejected, false);
}

static bool VehicleSiteMatches(DWORD rva, const BYTE* expected, size_t size)
{
    return rva <= g_exeSize && size <= g_exeSize - rva &&
           ReadablePtr(g_exeBase + rva, size) &&
           memcmp(g_exeBase + rva, expected, size) == 0;
}

static int WorkingVehicleDepositCount()
{
    int count = 0;
    for (int i = 0; i < g_depCount; i++) if (UsesWorkingVehicles(g_dep[i])) count++;
    return count;
}

static bool InstallWorkingVehiclePatch()
{
    const int count = WorkingVehicleDepositCount();
    if (!count) return true;  // omitted/none: no new native patches at all
    if (g_workingVehicleCave) return true;

    for (size_t i = 0; i < kWorkingVehicleSiteCount; i++)
    {
        const WorkingVehicleSite& site = kWorkingVehicleSites[i];
        if (!VehicleSiteMatches(site.rva, site.original, site.size) ||
            !VehicleSiteMatches(site.guardRva, site.guard, site.guardSize) ||
            site.acceptRva >= g_exeSize || site.rejectRva >= g_exeSize)
        {
            Logf("vehicles WARN %s at RVA 0x%lX differs from WRSR 1.1.1.9 "
                 "or is unreadable. All working-vehicle extensions disabled; no vehicle gates changed",
                 site.label, site.rva);
            return false;
        }
    }

    BYTE* cave = AllocNear(g_exeBase + kWorkingVehicleSites[0].rva, kWorkingVehicleCaveSize);
    if (!cave)
    {
        Logf("vehicles WARN cannot allocate nearby code; all working-vehicle extensions disabled");
        return false;
    }
    Emit e = { cave, cave + kWorkingVehicleCaveSize, false };
    BYTE* targets[kWorkingVehicleSiteCount] = {};
    for (size_t i = 0; i < kWorkingVehicleSiteCount; i++)
    {
        const WorkingVehicleSite& site = kWorkingVehicleSites[i];
        targets[i] = e.p;
        if (!VehicleRel32Fits(g_exeBase + site.rva + 5, targets[i])) e.overflow = true;
        EmitWorkingVehicleGate(e, site, g_exeBase + site.acceptRva, g_exeBase + site.rejectRva);
    }
    DWORD caveProtection = 0;
    if (e.overflow || !VirtualProtect(cave, kWorkingVehicleCaveSize, PAGE_EXECUTE_READ, &caveProtection) ||
        !FlushInstructionCache(GetCurrentProcess(), cave, kWorkingVehicleCaveSize))
    {
        Logf("vehicles WARN code preparation failed (size/range/protection/cache); "
             "no vehicle gates changed");
        VirtualFree(cave, 0, MEM_RELEASE);
        return false;
    }

    // These four sites occupy distinct pages in the verified executable. No
    // code is changed until all have passed both the byte and protection checks.
    DWORD oldProtection[kWorkingVehicleSiteCount] = {};
    size_t writable = 0;
    for (; writable < kWorkingVehicleSiteCount; writable++)
    {
        const WorkingVehicleSite& site = kWorkingVehicleSites[writable];
        if (!VirtualProtect(g_exeBase + site.rva, site.size, PAGE_EXECUTE_READWRITE,
                            &oldProtection[writable])) break;
    }
    if (writable != kWorkingVehicleSiteCount)
    {
        const DWORD error = GetLastError();
        Logf("vehicles WARN cannot make %s writable (%lu); no vehicle gates changed",
             kWorkingVehicleSites[writable].label, error);
        for (size_t i = 0; i < writable; i++)
        {
            DWORD ignored;
            const WorkingVehicleSite& site = kWorkingVehicleSites[i];
            if (!VirtualProtect(g_exeBase + site.rva, site.size, oldProtection[i], &ignored))
                Logf("vehicles WARN cannot restore %s page protection (%lu)", site.label, GetLastError());
        }
        VirtualFree(cave, 0, MEM_RELEASE);
        return false;
    }
    // Init-only installation: the loader invokes this before gameplay starts.
    for (size_t i = 0; i < kWorkingVehicleSiteCount; i++)
    {
        const WorkingVehicleSite& site = kWorkingVehicleSites[i];
        BYTE* at = g_exeBase + site.rva;
        const int displacement = (int)((intptr_t)targets[i] - (intptr_t)(at + 5));
        at[0] = 0xE9;
        memcpy(at + 1, &displacement, 4);
        memset(at + 5, 0x90, site.size - 5);
    }
    g_workingVehicleCave = cave; // installed jumps own the cave until process exit
    for (size_t i = 0; i < kWorkingVehicleSiteCount; i++)
    {
        const WorkingVehicleSite& site = kWorkingVehicleSites[i];
        DWORD ignored;
        if (!FlushInstructionCache(GetCurrentProcess(), g_exeBase + site.rva, site.size))
            Logf("vehicles WARN instruction-cache flush failed for %s (%lu)", site.label, GetLastError());
        if (!VirtualProtect(g_exeBase + site.rva, site.size, oldProtection[i], &ignored))
            Logf("vehicles WARN page-protection restore failed for %s (%lu)", site.label, GetLastError());
    }
    for (int i = 0; i < g_depCount; i++)
        if (UsesWorkingVehicles(g_dep[i]))
            Logf("vehicles [%s] type=%d -> GRAVELMINING (skill 3); "
                 "native purchase, assignment, work and arrival enabled; deposit map unchanged",
                 g_dep[i].name, g_dep[i].type);
    Logf("vehicles %d custom mine type(s), 4 native gates installed (%zu bytes); "
         "vanilla gravel/bauxite and vehicle fuel rules unchanged", count, (size_t)(e.p - cave));
    return true;
}
...
// a mod button needs no new art asset: the `icon` key in deposits_plus.ini names a
...
// from 1 000 000 up. But plugins load in directory order, `deposits_plus` comes before
...
// The registry, published for anything else that has to know what deposits_plus.ini
...
static const char* svc_Setting(int i, const char* key)
{
    ...
    const DepositDef* d = &g_dep[i];
    if (KeyIs(key, "working_vehicle_skill") && d->workingVehicleSkillSet)
        return d->workingVehicleSkill == 3 ? "gravelmining" : "none";
...
#include "deposit_visual_runtime.inl"
...
extern "C" __declspec(dllexport) int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info)
{
    ...
    // additive hooks and refuse the type patch, so when plugins\deposits.dll
    // exists and the launcher has it switched on, this plugin steps aside before
    // it touches anything - nothing is hooked yet, so unloading is legal. The
    // check runs whether the DLL was loaded from plugins\ or from a Workshop
    // package through workshop_bridge.
    {
        char upstream[MAX_PATH];
        _snprintf_s(upstream, sizeof(upstream), _TRUNCATE, "%s\\plugins\\deposits.dll", g_baseDir);
        if (GetFileAttributesA(upstream) != INVALID_FILE_ATTRIBUTES &&
            H->configInt("tesmioloader.ini", "plugins", "deposits", 1) != 0)
        {
    ...
    const char* ini = "plugins\\deposits_plus.ini";
    g_depositPatch = H->configInt(ini, "deposits_plus", "code_patch", g_depositPatch);
    g_minimapPatch = H->configInt(ini, "deposits_plus", "minimap",    g_minimapPatch);
    g_editorPatch  = H->configInt(ini, "deposits_plus", "editor",     g_editorPatch);
    ...
    LoadDepositRegistry();
    ValidateDeposits();
    if(g_generationConfigInvalid) g_generationEnabled=false;
    ...
    const bool depositTypesReady = g_depositPatch && PatchDepositType();
    if(depositTypesReady) InstallGeneration();
    if (depositTypesReady) InstallWorkingVehiclePatch();
    else if (WorkingVehicleDepositCount())
        Logf("vehicles WARN working-vehicle settings not activated: code_patch is disabled "
             "or the deposit type patch did not succeed");
...
{
    // The DLL's own folder, for assets that travel with it (Workshop package or
    // a local copy); resolved once, before Init can ask for a texture.
    if (reason == DLL_PROCESS_ATTACH)
    {
        char path[MAX_PATH];
        if (GetModuleFileNameA(module, path, MAX_PATH) > 0 && GetLastError() != ERROR_INSUFFICIENT_BUFFER)
        {
            char* slash = strrchr(path, '\\');
            if (slash) { *slash = 0; strncpy_s(g_selfDir, path, _TRUNCATE); }
        }
    }
    return TRUE;
}
