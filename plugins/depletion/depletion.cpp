...
#define SYM_TERRAIN_INIT "?InitializeFromFolder@C3D_TERRAIN@@QEAAHPEAVC3D_MIDDLEPOINT@@PEADPEAVC3D_LIGHTING@@PEAVC3D_WATER@@PEAVC3DAPI_SHADERS@@PEBDIH@Z"
...
#define P_LANGUAGE        0x997590   // C3D_LANGUAGE object, native mine panel uses it
...
typedef int    (*t_TerrainInit)(void*, void*, char*, void*, void*, void*, const char*, unsigned, int);
typedef const wchar_t* (*t_LanguageGetString)(void*, int);
...
static t_TerrainInit   o_TerrainInit;
static t_LanguageGetString o_LanguageGetString;
static int g_captionTextId;
static bool g_captionFallbackLogged;
...
{
    void*  building;
    void*  typeDesc;         // reject an address reused for a different building
    DWORD  lastSeen;
    int    state;
...
// Session-only state must never cross a world load. Pointer equality alone is
// insufficient: the game can reuse both terrain and building allocations.
// g_lock protects this lifecycle AND every access to the mine-state cache.
static unsigned long long g_worldEpoch;
static void*     g_worldTerrain;
static bool      g_worldInitializing;
static bool      g_worldReady;
static DWORD     g_lastFlush;

static bool CaptureWorld(unsigned long long* epoch)
{
    EnterCriticalSection(&g_lock);
    *epoch = g_worldEpoch;
    bool ready = g_enabled && g_worldReady;
    LeaveCriticalSection(&g_lock);
    return ready;
}

static int h_TerrainInit(void* self, void* mp, char* folder, void* lighting,
                         void* water, void* shaders, const char* shader,
                         unsigned flags, int detail)
{
    EnterCriticalSection(&g_lock);
    unsigned long long epoch = ++g_worldEpoch;
    g_worldReady = false;
    g_worldInitializing = true;
    g_worldTerrain = self;
    ...
    LeaveCriticalSection(&g_lock);
    Logf("deplete  world load: old mine states discarded; waiting for active terrain render");

    // Do not keep our lock around native code or another plugin's init hook.
    // If initialization faults, ready stays false; no stale cache is resumed.
    int result = o_TerrainInit(self, mp, folder, lighting, water, shaders,
                              shader, flags, detail);
    EnterCriticalSection(&g_lock);
    if (g_worldEpoch == epoch) g_worldInitializing = false;
    LeaveCriticalSection(&g_lock);
    return result;
}
...
static bool MineStillMatches(const MineState* st)
{
    BYTE* b = (BYTE*)st->building;
    if (!b || st->dep < 0 || st->dep >= g_depletableCount ||
        !ReadablePtr(b + B_TYPEDESC, sizeof(void*)) ||
        !ReadablePtr(b + B_QUALITY, sizeof(float))) return false;
    BYTE* type = *(BYTE**)(b + B_TYPEDESC);
    return type && type == st->typeDesc && ReadablePtr(type + T_BUILDING_TYPE, 12) &&
           *(int*)(type + T_BUILDING_TYPE) == 7 &&
           *(int*)(type + T_DEPOSIT_TYPE) == g_depletable[st->dep].type;
}
...
static void DepleteMine(BYTE* b)
{
    ...
    {
        st->dep = dep;
        st->typeDesc = typeDesc;
        InterlockedExchange(&g_pending, 1);      // the flusher will seed it
        return;
    }
    if (st->typeDesc != typeDesc || st->dep != dep)
    {
        memset(st, 0, sizeof(*st));
        st->building = b;
        st->typeDesc = typeDesc;
        st->dep = dep;
        st->lastSeen = g_mineTick;
        InterlockedExchange(&g_pending, 1);
        return;                                // reseed, never reuse another mine's reserve
    }
...
static void h_MineTick(void* game, void* building)
{
    ...
    __try
    {
        // A native tick started before a load must not debit the next world.
        if (!g_enabled || !g_worldReady || g_worldEpoch != epoch) return;
        if (!g_tickThread)
        {
    ...
    __finally { LeaveCriticalSection(&g_lock); }
}
...
static void FlushMap(int map)
{
    ...
    for (int i = 0; i < MAX_MINE_STATES; i++)
    {
        ...
        if (!MineStillMatches(st)) { memset(st, 0, sizeof(*st)); continue; }
    ...
    __finally { DepositTextureAccess(map, tex, false); }
}
...
static void h_TerrainRender(void* self, bool a, void* cam1, void* cam2, int b, int c)
{
    ...
    __try
    {
        if (g_enabled && !g_worldInitializing && self &&
            self == g_worldTerrain && self == TerrainObject())
        {
            if (!g_worldReady)
            {
                g_worldReady = true;
                Logf("deplete  world ready: mine reserves will be sampled from loaded maps");
            }
            ...
            if (InterlockedCompareExchange(&g_pending, 0, 1) == 1 ||
                (DWORD)(now - g_lastFlush) > (DWORD)(g_flushSeconds * 1000.0f))
            {
                if ((DWORD)(now - g_lastFlush) > 250)      // never more than 4 per second
                {
                    g_lastFlush = now;
                    __try { FlushAll(); }
...
static void ResolvePanelLocalization()
{
    g_captionTextId = 0;
    g_captionFallbackLogged = false;
    o_LanguageGetString = NULL;
    if (!g_panel || !H || !H->consume) return;
    const TsmLocalizationApi* localization = (const TsmLocalizationApi*)H->consume(
        TSM_SERVICE_LOCALIZATION, TSM_LOCALIZATION_VERSION);
    if (!localization || !localization->resolveFull)
    {
        Logf("deplete  localization unavailable; existing panel_caption retained");
        return; // optional text service must never disable depletion
    }
    int id = localization->resolveFull("tesmio_lang.depletion.deposit_remaining");
    if (id < 2000000 || id > 2999999)
    {
        Logf("deplete  localization key tesmio_lang.depletion.deposit_remaining missing/invalid (id=%d); panel_caption retained", id);
        return;
    }
    void** slot = FindIatSlot(g_exe, DLL_ENGINE, "?GetString@C3D_LANGUAGE@@QEAAPEA_WH@Z");
    if (!slot || !*slot || !g_exeBase || g_exeSize <= P_LANGUAGE)
    {
        Logf("deplete  native language lookup unavailable; panel_caption retained");
        return;
    }
    o_LanguageGetString = (t_LanguageGetString)*slot;
    g_captionTextId = id;
    Logf("deplete  localization key tesmio_lang.depletion.deposit_remaining resolved to %d; fallback=panel_caption", id);
}

static bool CopyPanelCaption(const wchar_t* text, wchar_t* out, size_t capacity)
{
    if (!text || !out || capacity < 2 || !text[0]) return false;
    for (size_t i = 0; i < capacity; ++i)
    {
        wchar_t c = text[i];
        if (!c) { out[i] = 0; return true; }
        // This is a single-row caption, not a format string or a multiline UI.
        if (i + 1 == capacity || c < L' ' || c == 0x7F) return false;
        out[i] = c;
    }
    return false;
}

static void PanelCaption(wchar_t* out, size_t capacity)
{
    if (!out || capacity < 2) return;
    bool translated = false;
    if (g_captionTextId && o_LanguageGetString)
    {
        __try
        {
            // Fetch from the active game language each time; never retain a
            // language-owned text pointer across a reload or language switch.
            translated = CopyPanelCaption(o_LanguageGetString(
                g_exeBase + P_LANGUAGE, g_captionTextId), out, capacity);
        }
        __except (FaultFilter("depletion optional panel localization", GetExceptionInformation()))
        {
            g_captionTextId = 0; // isolate a failed lookup, keep the panel/gameplay active
        }
        if (!translated && !g_captionFallbackLogged)
        {
            g_captionFallbackLogged = true;
            Logf("deplete  localized caption unavailable/too long/not single-line; using panel_caption");
        }
    }
    if (translated) return;
    out[0] = 0;
    if (!MultiByteToWideChar(CP_ACP, 0, g_panelCaption, -1, out, (int)capacity))
        wcsncpy_s(out, capacity, L"Deposit remaining", _TRUNCATE);
}
...
static void h_MinePanel(void* game, void* window, void* p3)
{
    ...
    if (!ready || !g_panel || !window) return;
...
extern "C" __declspec(dllexport) int TsmPluginStart(void)
{
    ...
    ResolvePanelLocalization();

    // A lifecycle guard is mandatory: a render hook alone would reuse old
    // building pointers after loading another world in the same process.
    if (!FindIatSlot(g_exe, DLL_ENGINE, SYM_TERRAIN_INIT) ||
        !FindIatSlot(g_exe, DLL_ENGINE, SYM_TERRAIN_RENDER))
    {
        Logf("deplete  terrain init/render import missing - not hooking");
        return 1;
    }
    if (!PatchIat(g_exe, DLL_ENGINE, SYM_TERRAIN_INIT, (void*)h_TerrainInit,
                  (void**)&o_TerrainInit, "depletion terrain init")) return 1;

    // After the first installed hook the DLL must remain loaded even if a
    // later hook fails; disable our work but preserve the native hook chain.
    if (!PatchIat(g_exe, DLL_ENGINE, SYM_TERRAIN_RENDER, (void*)h_TerrainRender,
                  (void**)&o_TerrainRender, "C3D_TERRAIN::Render"))
    {
        Logf("deplete  render hook failed - disabled, keeping installed init hook resident");
        g_enabled = 0;
        return 0;
    }
    ...
    {
        Logf("deplete  mine tick hook failed - disabled, keeping terrain hooks resident");
        g_enabled = 0;
        return 0;
    }
